...
// ── Ricky Bobby chat history (Step 17) ────────────────────────────────────
// One saved conversation per row, private to its owner (RLS). Title is the
// first question, trimmed (lib/ricky-chat.ts).
export type RickyChat = {
  id: string
  user_id: string
  title: string
  created_at: string
...
export type RickyChatSummary = Pick<RickyChat, 'id' | 'title' | 'updated_at'>

export type RickyMessage = {
  id: string
  chat_id: string
  role: 'user' | 'assistant'
  content: string
  // Assistant turns: what was retrieved for the answer. Null on user turns.
  sources: RickyRetrievedSource[] | null
  created_at: string
}
